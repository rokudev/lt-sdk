/******************************************************************************
 * bl61x.c                                    Bouffalo Labs bl61x Flash Support
 *
...
enum {
    ...
    kDefaultSerialTimeoutMS        = 500,
    kChecksumSerialTimeoutMS       = 4000,
    kEraseSerialTimeoutMS          = 10000,
};
...
typedef struct {
    u8   uartIRQen[4];
    u8   baudRate[4];
    //u8   clockParams[20];  // non-essential
} DeviceConfig;

typedef struct {
    /* 0x80: auto scan */
    u8 flashPin;
    /* bit 7-4: flash_clock_type: 0: WIFIPLL_120M, 1: XTAL, 2: WIFIPLL_48M, 3: WIFIPLL_80M, 4: BCLK, 5: WIFIPLL_96M
...
static const u8 s_flashParams[kFlashParams_Total][84] = {
    ...
    { /* Idx 2 */
        0x04, 0x01, 0x00, 0x00, 0x66, 0x99, 0xff, 0x03, 0x9f, 0x00, 0xb7, 0xe9, 0x04, 0xc8, 0x00, 0x01,
        0xc7, 0x20, 0x52, 0xd8, 0x06, 0x02, 0x32, 0x00, 0x0b, 0x01, 0x0b, 0x01, 0x3b, 0x01, 0xbb, 0x00,
        0x6b, 0x01, 0xeb, 0x02, 0xeb, 0x02, 0x02, 0x50, 0x00, 0x01, 0x00, 0x01, 0x01, 0x00, 0x02, 0x01,
        0x02, 0x01, 0xab, 0x01, 0x05, 0x35, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x38, 0xff, 0xa0, 0xff,
        0x77, 0x03, 0x02, 0x40, 0x77, 0x03, 0x02, 0xf0, 0x2c, 0x01, 0xb0, 0x04, 0xb0, 0x04, 0x05, 0x00,
...
/*
        ...
        0x80: scan the flash pin automatically
        0x10: select XTAL as flash clock, without frequency division
...
static int TargetInit(bool bAutoProgram) {
    ...
    // Make sure no data cached before communication
    SerialFlush();
    int nRtn;
    DeviceInfo info;
    ...
    if (nRtn < 0) return nRtn;
    /* Send full flash configuration (88 bytes: 4 initial + 84 params) */
    /* This is REQUIRED for flash operations to work correctly */
    ...
    if (nRtn < 0) return nRtn;
    nRtn = SendCommandAndWaitForOK(kCommandID_SetFlashConfig,
                                   (u8 *)&flashConfig, sizeof(flashConfig));
    if (nRtn < 0) return nRtn;
    return nRtn;
}
...
static int EraseFlash(Area * pArea, Image * pImage) {
    ...
    int nRtn = SerialSetTimeout(kEraseSerialTimeoutMS);
    if (nRtn < 0) return nRtn;
    nRtn = SendCommandAndWaitForOK(kCommandID_FlashErase, (u8 *)&erase, sizeof(erase));
    int restoreRtn = SerialSetTimeout(kDefaultSerialTimeoutMS);
    if (restoreRtn < 0) return restoreRtn;
    return nRtn;
}
...
FlashDeviceInterface FlashDeviceInterface_bl61x = {
    &Open,
    &Close,
    &Program,
    &Read,
...
const FlashDeviceInterfaceMapEntry FlashDeviceMapEntry_bl61x = {
    .pFamily = "bl61x",
    .pDevice = NULL,
    .pLegacyDevice = NULL,
    .pInterface = &FlashDeviceInterface_bl61x
};

static void LT_USED_CONSTRUCTOR FlashDevice_RegisterSelf_bl61x(void) {
    (void)FlashDevice_Register(&FlashDeviceMapEntry_bl61x);
}
