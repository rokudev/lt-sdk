...
static int SetSearchPath(char * pPaths) {
    ...
    if (pPaths) {
        ...
        char * pPlatformBuildVariantDir = getenv("LT_PLATFORM_BUILD_PLATFORM_VARIANT_DIR");
        char * pPlatform     = getenv("LT_PLATFORM");
        char * pProductRoot  = getenv("LT_PRODUCT_ROOT");
        char * pProductBuildVariantDir = getenv("LT_PRODUCT_BUILD_PRODUCT_VARIANT_DIR");
        char * pProduct      = getenv("LT_PRODUCT");
        if (pPlatformRoot && (pPlatformBuildVariantDir || pPlatform)) {
            ...
            if (nRtn == 0) {
                ...
                size -= len;
                if (pProductBuildVariantDir) {
                    len = lt_snprintf(s_searchPaths + len, size, ":%s/build/image:%s:.", pPlatformRoot, pProductBuildVariantDir);
                }
                else if (pProductRoot && pProduct) {
                    len = lt_snprintf(s_searchPaths + len, size, ":%s/build/image:%s/build/product/%s:.", pPlatformRoot, pProductRoot, pProduct);
                }
                else {
                    len = lt_snprintf(s_searchPaths + len, size, ":%s/build/image:.", pPlatformRoot);
                }
                if (len < 0 || len >= size) nRtn = -1;
            }
            if (nRtn == -1) printf("Error creating search paths\n");
        } else {
