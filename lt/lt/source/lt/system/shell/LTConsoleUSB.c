/******************************************************************************
 * lt/source/system/shell/LTConsoleUSB.c
 *
...
/*_________________________
  ...
  _____________
  LTConsoleUSB - LTObject that turns on, off and redirects USB-I/O to LTCore's Console I/O; it requires no api so declares the base LTObject as the api it implements */
typedef_LTObject(LTConsoleUSB, 1) { } LTOBJECT_API;

typedef_LTObjectImpl(LTConsoleUSB, LTConsoleUSBImpl) {
...
static LTConsoleUSBImpl * s_pConsoleUSB   = NULL;
...
static void LTConsoleUSB_ConsolePutCharProc(const char * pChars, u32 nChars, void * pClientData) {
   ...
   if (s_pConsoleUSB && (s_pConsoleUSB == (LTConsoleUSBImpl *)pClientData) && pChars && nChars) {
       if (! bInISR) {
...
static void OnUsbReadReady(void * pClientData) {
    ...
    if (s_pConsoleUSB && (s_pConsoleUSB == (LTConsoleUSBImpl *)pClientData)) {
...
static void OnUsbWriteReady(void * pClientData) {
    LT_UNUSED(pClientData);
    s_pMutex->API->Lock(s_pMutex);
    if (s_pConsoleUSB && (s_pConsoleUSB == (LTConsoleUSBImpl *)pClientData)) FlushWriteBufferInternal();
    s_pMutex->API->Unlock(s_pMutex);
}
...
/*______________________________________
  LTConsoleUSB object constructors */
static void LTConsoleUSBImpl_DestructObject(LTConsoleUSBImpl * pConsoleUSB);
static bool LTConsoleUSBImpl_ConstructObject(LTConsoleUSBImpl * pConsoleUSB) {
    ...
    do {
        ...
        pConsoleUSB->nBytesInWriteBuffer += LTShellImpl_GetShellBanner((char *)(pConsoleUSB->pUSBWriteBuffer + pConsoleUSB->nBytesInWriteBuffer), pConsoleUSB->nWriteBufferSize - pConsoleUSB->nBytesInWriteBuffer);
    ...
    if (s_pConsoleUSB) {
        ...
        */
        ...
        LTConsoleUSBImpl_DestructObject(pConsoleUSB);
    }
...
EnableAuxiliaryUSBConsole(void) {
    s_pMutex->API->Lock(s_pMutex);
    bool bSuccess = s_pConsoleUSB ? true : ((s_pConsoleUSB = (LTConsoleUSBImpl *)lt_createobject_typed(LTConsoleUSB, LTConsoleUSBImpl)) ? true : false);
    s_pMutex->API->Unlock(s_pMutex);
    return bSuccess;
}
...
DisableAuxiliaryUSBConsole(void) {
 /* to disable the auxiliary usb console we simply destroy s_pConsoleUSB, but we have to be careful,
    ...
    s_pMutex->API->Lock(s_pMutex);
    LT_SIZE nMask = LT_GetCore()->Disable(); LTConsoleUSBImpl * pConsoleUSB = s_pConsoleUSB; s_pConsoleUSB = NULL; LT_GetCore()->Enable(nMask);
    if (pConsoleUSB) {
...
define_LTObjectImplPrivate(LTConsoleUSB, LTConsoleUSBImpl);


/******************************************************************************
 *  LOG
 ******************************************************************************
 *  07--22      augustus    updated for thread and event api enhancements
 */
