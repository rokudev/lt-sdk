...
static int16_t lmagn[HALF_ANAL_BLOCKL];
...
static int16_t LT_ALIGNED(32) winData_buff[ANAL_BLOCKL_MAX * 2 + 16];
static int16_t LT_ALIGNED(32) realImag_buff[ANAL_BLOCKL_MAX * 2 + 16];
...
void WebRtcNsx_DataAnalysis(NoiseSuppressionFixedC* inst,
  ...
  int16_t* winData = (int16_t*) winData_buff; //(((uintptr_t)winData_buff + 31) & ~31);
  int16_t* realImag = (int16_t*) realImag_buff; //(((uintptr_t)realImag_buff + 31) & ~31);
...
static int16_t LT_ALIGNED(32) realImag_buff[ANAL_BLOCKL_MAX * 2 + 16];
static int16_t LT_ALIGNED(32) rfft_out_buff[ANAL_BLOCKL_MAX * 2 + 16];
...
void WebRtcNsx_DataSynthesis(NoiseSuppressionFixedC* inst, short* outFrame) {
  ...
  // Align the structures to 32-byte boundary for the FFT function.
  int16_t* realImag = (int16_t*) realImag_buff; //(((uintptr_t)realImag_buff + 31) & ~31);
  int16_t* rfft_out = (int16_t*) rfft_out_buff; //(((uintptr_t)rfft_out_buff + 31) & ~31);
...
static uint32_t noiseU32[HALF_ANAL_BLOCKL];
static uint32_t postLocSnr[HALF_ANAL_BLOCKL];
static uint32_t priorLocSnr[HALF_ANAL_BLOCKL];
static uint32_t prevNearSnr[HALF_ANAL_BLOCKL];
static uint16_t magnU16[HALF_ANAL_BLOCKL];
static uint16_t prevNoiseU16[HALF_ANAL_BLOCKL];
static uint16_t nonSpeechProbFinal[HALF_ANAL_BLOCKL];
static uint16_t noiseSupFilterTmp[HALF_ANAL_BLOCKL];
