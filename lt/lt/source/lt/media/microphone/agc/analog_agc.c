...
DEFINE_LTLOG_SECTION("wrtc.alog.agc")
...
int WebRtcAgc_AddMic(void* state,
  ...
  if (stt->micVol > stt->maxAnalog) {
    ...
    if (stt->maxLevel <= stt->maxAnalog) {
      LTLOG_YELLOWALERT("add.mic.level", "maxLevel %ld not more than maxAnalog %ld",
          LT_Ps32(stt->maxLevel), LT_Ps32(stt->maxAnalog));
    }
    ...
    if (targetGainIdx >= GAIN_TBL_LEN) {
      LTLOG_YELLOWALERT("add.mic.tgt.idx", "targetGainIdx %u more than GAIN_TBL_LEN %d",
          targetGainIdx, GAIN_TBL_LEN);
    }
...
void WebRtcAgc_UpdateAgcThresholds(LegacyAgc* stt) {
  ...
  if (stt->agcMode == kLTMediaMicrophoneAGC_Mode_FixedDigital) {
...
int32_t WebRtcAgc_ProcessAnalog(void* state,
  ...
  if (stt->firstCall == 0) {
    ...
    if ((inMicLevelTmp < tmpVol) && (stt->agcMode == kLTMediaMicrophoneAGC_Mode_AdaptiveAnalog)) {
...
int WebRtcAgc_Analyze(void* agcInst,
  ...
  *outMicLevel = inMicLevel;
  ...
  if (stt->agcMode < kLTMediaMicrophoneAGC_Mode_FixedDigital &&
      (stt->lowLevelSignal == 0 || stt->agcMode != kLTMediaMicrophoneAGC_Mode_AdaptiveDigital)) {
...
int WebRtcAgc_set_config(void* agcInst, LTMediaMicrophoneAGC_Config agcConfig) {
  LegacyAgc* stt;
  stt = (LegacyAgc*)agcInst;
  ...
  /* Store the config in a LTMediaMicrophoneAGC_Config */
...
int WebRtcAgc_get_config(void* agcInst, LTMediaMicrophoneAGC_Config* config) {
  LegacyAgc* stt;
  stt = (LegacyAgc*)agcInst;
 ...
 */
  if (agcMode < kLTMediaMicrophoneAGC_Mode_Unchanged || agcMode > kLTMediaMicrophoneAGC_Mode_FixedDigital) {
    return -1;
  }
  stt->agcMode = agcMode;
  stt->fs = fs;
  ...
  if (stt->agcMode == kLTMediaMicrophoneAGC_Mode_AdaptiveDigital) {
    //minLevel = 0;
    //maxLevel = 255;
    stt->scale = 0;
  }
...
/* micVol     - Microphone volume level
 */
void WebRtcAgc_SetMicVol(void* agcInst,
                   int32_t micVol) {
  LegacyAgc* stt;
  ...
  if (stt->agcMode == kLTMediaMicrophoneAGC_Mode_AdaptiveDigital) {
    stt->micVol = LT_MIN(micVol, stt->maxAnalog);
    stt->micGainIdx = stt->micVol;
  }
}
