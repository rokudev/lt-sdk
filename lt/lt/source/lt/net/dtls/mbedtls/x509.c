...
{
    LTCalendarTime calTime;
    LTTime timeNow = LT_GetCore()->GetClockTimeUTC();
    LTSystemTimeZone *timezone = lt_openlibrary(LTSystemTimeZone);
    if (!timezone) {
        return -1;
    }
    timezone->ClockTimeToCalendarTime(timeNow, &calTime);
    now->year = calTime.nYear + 1900;
    now->mon  = calTime.nMonth  + 1;
    now->day  = calTime.nDay;
    now->hour = calTime.nHour;
    now->min  = calTime.nMinute;
    now->sec  = calTime.nSecond;
