...
start_processing:
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        LT_UNUSED(rs_ctx);
        ret = LTNetDtls_VerifySigHash(peer_pk, hash, p, sig_len) ? 0 : MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        if (ret != 0) {
...
sign:
    ...
    LT_UNUSED(rs_ctx); LT_UNUSED(md_alg);
    if (0 == (n = LTNetDtls_SignCertVerify(ssl->clientPrivateKeyReference, hash_start, ssl->out_msg + 6 + offset, out_buf_len - 6 - offset))) {
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
