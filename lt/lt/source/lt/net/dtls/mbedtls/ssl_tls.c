...
#if defined(MBEDTLS_SSL_CERT_VERIFY_BY_DIGEST)
int mbedtls_ssl_conf_peer_cert_digest(mbedtls_ssl_config *conf,
                                      mbedtls_md_type_t peer_cert_digest_type,
                                      const unsigned char *peer_cert_digest,
    ...
    if (digest_len != peer_cert_digest_len) {
        return -1;
    }

    conf->peer_cert_digest_type = peer_cert_digest_type;
    conf->peer_cert_digest_len = peer_cert_digest_len;
    memcpy(conf->peer_cert_digest, peer_cert_digest, digest_len);
    return 0;
}
#endif
...
} tls_id_match_table[] =
{
    ...
    { 23, MBEDTLS_ECP_DP_SECP256R1, 0, 256, EC_NAME("secp256r1") },
#endif
#if defined(MBEDTLS_ECP_DP_SECP256K1_ENABLED) || defined(PSA_WANT_ECC_SECP_K1_256)
    { 22, MBEDTLS_ECP_DP_SECP256K1, 0, 256, EC_NAME("secp256k1") },
#endif
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> write certificateeeee"));
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("own certificate"));
    /*
     *     0  .  0    handshake type
    ...
    while (crt != NULL) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("crt not NULL"));
        n = crt->raw.len;
    ...
    ssl->state++;
    MBEDTLS_SSL_DEBUG_MSG(2, ("crt done"));
    if ((ret = mbedtls_ssl_write_handshake_msg(ssl)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_write_handshake_msg", ret);
        return ret;
    }
...
#if defined(MBEDTLS_SSL_CERT_VERIFY_BY_DIGEST)
    if ((ssl->session_negotiate->peer_cert_digest_len  != ssl->conf->peer_cert_digest_len) ||
        (ssl->session_negotiate->peer_cert_digest_type != ssl->conf->peer_cert_digest_type) ||
        memcmp(ssl->session_negotiate->peer_cert_digest, ssl->conf->peer_cert_digest, ssl->session_negotiate->peer_cert_digest_len) != 0) {
        ret = MBEDTLS_ERR_SSL_BAD_CERTIFICATE;
        goto exit;
    }
#endif
