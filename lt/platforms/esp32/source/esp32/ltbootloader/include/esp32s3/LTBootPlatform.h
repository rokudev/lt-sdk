...
/* The transmitter is busy while bytes remain in the Tx FIFO or the shift register
 * is still clocking one out.  This part reports the FIFO count in UART_STATUS but
 * keeps the transmitter state in a separate FSM_STATUS register, so it takes two
 * reads rather than the esp32's one. */
#define UART_TX_FIFO_BUSY                                                            \
    ((ESP32_UART_REG(0, STATUS)     & ESP32_REG_MASK(UART_STATUS, TXFIFO_CNT)) ||    \
     (ESP32_UART_REG(0, FSM_STATUS) & ESP32_REG_MASK(UART_FSM_STATUS, UTX_OUT)))
