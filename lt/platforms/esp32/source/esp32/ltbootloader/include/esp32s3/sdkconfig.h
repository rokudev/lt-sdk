...
/* Chip identity.  IDF generates one sdkconfig.h per target; this file stands in
 * for that, and this platform root builds for exactly one part.
 *
 * The bootloader offset is 0 and not the esp32's 0x1000: the esp32s3 first stage
 * ROM loader reads the second stage image from the very start of flash. */
...
#define CONFIG_SECURE_BOOTLOADER_VERSION 0
...
#define CONFIG_SECURE_FLASH_UART_BOOTLOADER_ALLOW_DEC 1
...
/* Where the bootloader's own log goes.
 *
 * The esp32s3 has a USB full speed PHY with a CDC-ACM device behind it, and
 * boards built on it generally bring D+/D- out to their only connector and fit
 * no USB to UART bridge, which leaves UART0 on GPIO43/44 with nothing attached
 * to it.  Pointing the ROM's printf at the USB serial/JTAG device instead costs
 * one store - the ROM has already enumerated it to print its own banner - and is
 * what puts the bootloader log on the same cable that powers and flashes the
 * board.  It also carries into the application: LT's BSP console is the same
 * device, and esp_rom_printf from Esp32_LTChipStart follows UartDev, which the
 * application inherits from the bootloader untouched.
 *
 * See bootloader_console.c for the implementation this selects. */
