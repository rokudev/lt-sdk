...
{
    /* LOCAL PATCH (not upstream): the esp32s3 MWDT has no edge interrupt, so
       upstream leaves this a no-op with both parameters unused.  LT builds with
       -Wextra -Werror, which upstream does not, so discard them explicitly. */
    (void)hw;
    (void)enable;
}
