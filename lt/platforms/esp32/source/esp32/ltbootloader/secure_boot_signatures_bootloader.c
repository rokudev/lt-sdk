...
static esp_err_t get_secure_boot_key_digests(esp_image_sig_public_key_digests_t *public_key_digests)
{
    ...
    LTBootSecurityCheck check = GetApplicationSecureBootKeyDigest(&trusted_keys.key_digests[0]);

    // Create the copies for FI checks (assuming result is ETS_OK, if it's not then it'll fail the fault check anyhow)
    GetApplicationSecureBootKeyDigest(&trusted_key_copies[0].key_digests[0]);
    GetApplicationSecureBootKeyDigest(&trusted_key_copies[1].key_digests[0]);
    LTBootPlatform_SecurityAssert(memcmp(&trusted_keys, &trusted_key_copies[0], sizeof(ets_secure_boot_key_digests_t)) == 0);
    LTBootPlatform_SecurityAssert(memcmp(&trusted_keys, &trusted_key_copies[1], sizeof(ets_secure_boot_key_digests_t)) == 0);

    if (check == kLTBootSecurityCheck_Pass) {
