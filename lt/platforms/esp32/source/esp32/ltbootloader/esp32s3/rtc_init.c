...
#include <sys/param.h>  // For MIN, which upstream picks up by way of spi_flash_os.h
#ifndef BOOTLOADER_BUILD
/* No symbol declared by spi_flash_os.h is referenced from the bootloader path, and
   the spi_flash component it belongs to is not vendored here, so the include joins
   sar_periph_ctrl.h behind the app-stage guard. */
