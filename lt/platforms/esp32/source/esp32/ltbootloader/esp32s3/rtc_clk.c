...
void rtc_clk_bbpll_configure(rtc_xtal_freq_t xtal_freq, int pll_freq)
{
    ...
    /*
     * LT change: bounded.  Upstream spins here forever.
     *
     * IDF only reaches this on a cold boot or a CPU-only reset, but LT's reboot
     * is SW_SYS_RST, which reports RESET_REASON_CORE_SW.  The skip in
     * bootloader_clock_configure() is conditional on RESET_REASON_CPU0_SW, so LT
     * re-runs rtc_clk_init() on every warm boot - and gets here with a BBPLL that
     * was never powered down, because the reboot path deliberately leaves it
     * running to keep the USB Serial/JTAG console enumerated across the reset.
     *
     * A completed calibration leaves BBPLL_STOP_FORCE_HIGH set, and the REGI2C
     * writes above rewrite the values the PLL already holds, so on that path
     * there is no new calibration to finish and CAL_DONE need not assert again.
     * Whether it does varies run to run, which is what made "reboot" hang
     * intermittently with nothing on the console - this is upstream of
     * bootloader_console_init(), so a wedge here is silent.
     *
     * Falling through on timeout is the right response to that case: the PLL is
     * locked at the frequency just requested and the calibration was a no-op.  In
     * the case this cannot distinguish - a PLL genuinely failing to calibrate -
     * carrying on hands the problem to the watchdog, which beats hanging here
     * where nothing can report it.  10ms is several orders of magnitude beyond a
     * real calibration.
     */
    for (int i = 0; i < 10000; i++) {
        if (GET_PERI_REG_MASK(I2C_MST_ANA_CONF0_REG, I2C_MST_BBPLL_CAL_DONE)) break;
        esp_rom_delay_us(1);
    }
    esp_rom_delay_us(10);
...
static void rtc_clk_cpu_freq_to_pll_mhz(int cpu_freq_mhz)
{
     ...
     */
    /* cpu_freq_mhz is only ever 80, 160 or 240 here, so widening it to match the
       unsigned freq_mhz is value preserving.  LT builds with -Wextra -Werror, which
       upstream does not. */
    if ((uint32_t)cpu_freq_mhz > cur_config.freq_mhz) {
        if (cpu_freq_mhz == 240) {
