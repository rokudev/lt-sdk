...
static esp_err_t check_and_generate_secure_boot_keys(const esp_image_metadata_t *image_data)
{
    ...
    /* Confirm digest matches application digest */

    ets_secure_boot_key_digests_t trusted_keys;
    GetApplicationSecureBootKeyDigest(&trusted_keys.key_digests[0]);

    if (memcmp((uint8_t *)trusted_keys.key_digests[0], app_key_digests.key_digests[0], ESP_SECURE_BOOT_DIGEST_LEN)) {
        ESP_LOGE(TAG, "Application key digest does not match");
        return ESP_FAIL;
    }
...
esp_err_t esp_secure_boot_v2_permanently_enable(const esp_image_metadata_t *image_data)
{
    ...
    LTBootPlatform_printf("Secure boot enabled!\n");
