...
try:
    ...
    #
    # ROKU
    #       Do not raise upon failure to import pyserial.  Doing so causes image
    #       mastering to fail on the build server, where pyserial is not
    #       available.  Instead, make an empty serial to cause uses thereof
    #       to fail more gracefully downstream.
    #       Many thanks to Mike Skeffington for Python expertise.
    #
    serial = dict()
    #
    # ROKU
...
try:
    ...
    print("The installed version of pyserial appears to be too old for esptool.py (Python interpreter %s). "
          "Check the README for installation instructions." % (sys.executable))
    #
    # ROKU
    #       Do not raise upon failure to import the port list.
    #       Also (above), do not report the version of pyserial (as sys.VERSION)
    #       as this call was failing as undefined on the build server.
    #       Many thanks to Mike Skeffington for Python expertise.
    #
    list_ports = None
    #
...
class ESPLoader(object):
    ...
    def sync(self):
        ...
        # probably indicates that the chip wasn't or couldn't be reset properly and esptool is talking to the
        # flasher stub.
...
class ESP32FirmwareImage(BaseFirmwareImage):
    ...
    def save_extended_header(self, save_file):
        ...
        fields = [self.wp_pin,
                  ...
                  self.min_rev]
        fields += [self.fw_ver_major]
        fields += [self.fw_ver_minor]
        fields += [0] * 6  # padding
...
def elf2image(args):
    ...
    # build-version
    fw_version = list(args.build_version)
    if len(fw_version) != 3 or fw_version[1] != '.':
        raise FatalError('Build version format must be #.#')
    image.fw_ver_major = int(fw_version[0])
    image.fw_ver_minor = int(fw_version[2])
    if image.fw_ver_major < 0 or image.fw_ver_major > 9 or image.fw_ver_minor < 0 or image.fw_ver_minor > 9:
        raise FatalError('Build version format must be #.#')
...
def main(argv=None, esp=None):
    ...
    parser_elf2image.add_argument('--use_segments', help='If set, ELF segments will be used instead of ELF sections to genereate the image.',
                                  action='store_true')
    parser_elf2image.add_argument('--build-version', help=('Build version "#.#".'), type=str, default="0.0");
