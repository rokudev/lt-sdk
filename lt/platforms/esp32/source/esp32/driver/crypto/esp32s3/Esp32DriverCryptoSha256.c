/******************************************************************************
 * platforms/esp32/source/esp32/driver/crypto/esp32s3/Esp32DriverCryptoSha256.c>
 *
...
/*
 * The esp32s3 SHA peripheral differs from the esp32's in three ways that matter
 * here.  One engine serves every algorithm and is steered by MODE, rather than
 * there being a start/continue/load/busy quad per algorithm.  The digest lives in
 * its own register window (H) instead of being loaded back over the message
 * window, so there is no load step - it is readable as soon as BUSY clears.  And
 * the engine takes the message, and returns the digest, in the natural byte order
 * of the buffer, where the esp32 needs each word swapped in both directions.
 */
...
static LTSystemCryptoResult HashBlock(LT_SHA256_CTX *ctx, const u8 block[SHA256_BLOCK_LENGTH]) {
    ...
    ESP32_REG(SHA_MODE) = ESP32_REG_VAL(SHA_MODE, SHA256);
    if (ctx->bFirst) {
        // start hashing
        ESP32_REG(SHA_START) = 1;
        ctx->bFirst = false;
    } else {
        // continue hashing
        ESP32_REG(SHA_CONTINUE) = 1;
    }
    if (!WaitIdle()) {
...
static LTSystemCryptoResult ReadDigest(u8 digest[SHA256_HASH_LENGTH], u32 h[SHA256_HASH_LENGTH / 4]) {
    ...
    volatile u32 *reg = ESP32_REG_ADDR(SHA_H);
    int i;
