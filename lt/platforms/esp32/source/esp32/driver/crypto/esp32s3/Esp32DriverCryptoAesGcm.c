/*******************************************************************************
 * platforms/esp32/source/esp32/driver/crypto/esp32s3/Esp32DriverCryptoAesGcm.c>
 *
...
static bool WaitIdle(void) {
    ...
    while (ESP32_REG(AES_STATE) != ESP32_REG_VAL(AES_STATE, IDLE) && !bTimeout) {
        bTimeout = LTTime_IsGreaterThan(LTTime_Subtract(LT_GetCore()->GetKernelTime(), t), ESP32_TIMEOUT_SHORT);
    }
    return (ESP32_REG(AES_STATE) == ESP32_REG_VAL(AES_STATE, IDLE));
}
...
static LTSystemCryptoResult AES128_Block_Encrypt(const u8 plaintext[AES128_BLOCK_LENGTH], u8 ciphertext[AES128_BLOCK_LENGTH]) {
    CopyToReg(ESP32_REG_ADDR(AES_TEXT_IN), plaintext);

    ESP32_REG(AES_TRIGGER) = 1;
    if (!WaitIdle()) {
        return kLTSystemCrypto_Result_Error;
    }

    lt_memcpy(ciphertext, (u32 *)ESP32_REG_ADDR(AES_TEXT_OUT), AES128_BLOCK_LENGTH);
...
static LTSystemCryptoResult AES128_GCM_Init(ESP32_AES128_GCM_CTX *ctx, const u8 key[AES128_KEY_LENGTH], const u8 *iv, LT_SIZE ivLen, const u8 *aad, LT_SIZE aadLen) {
    ...
    /* Redundant after a clock enable, which resets the engine, but the GCM
     * construction below depends on raw single block ECB. */
    ESP32_REG(AES_DMA_ENABLE) = 0;
    ESP32_REG(AES_BLOCK_MODE) = ESP32_REG_VAL(AES_BLOCK_MODE, ECB);
