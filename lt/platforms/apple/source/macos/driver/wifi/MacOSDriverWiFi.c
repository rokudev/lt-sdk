...
DEFINE_LTLOG_SECTION("macos.drv.wifi");
...
typedef struct WiFiUnit {
    ...
    LTMutex *mutex;
    LTAtomic txFrameCount;
    LTAtomic rxFrameCount;
    union {
...
static LTWiFi_DriverInfo DriverInfo = {
    ...
    .version  = "0.0.1",
    .updated  = "2022-2-21",
    .mac_address.octet = { 0x00, 0x0D, 0x4B, 0x01, 0x02, 0x03 },
};
...
define_LTDEVICE_DRIVER_IMPLEMENTATION(LTDriverWiFi, MacOSDriverWiFi);
/*
...
static bool MacOSDriverWiFiImpl_LibInit(void) {
    pCore = LT_GetCore();
...
static void MacOSDriverWiFiImpl_LibFini(void) {
...
static u32 MacOSDriverWiFiImpl_GetNumDeviceUnits(void) {
...
static LTDeviceUnit MacOSDriverWiFiImpl_CreateDeviceUnitHandle(u32 nDeviceUnitNumber) {
...
static void DriverWiFi_GetMetrics(LTDeviceUnit h_unit, LTWiFi_Metrics *metrics, LT_SIZE sizeOfMetrics) {
    ...
    if (metrics) {
        metrics->tx_frame_count = LTAtomic_Load(&unit->txFrameCount);
        metrics->rx_frame_count = LTAtomic_Load(&unit->rxFrameCount);
    }
    else {
        LTAtomic_Store(&unit->txFrameCount, 0);
        LTAtomic_Store(&unit->rxFrameCount, 0);
    }
}
...
static void DriverWiFi_JoinCheck(LTDeviceUnit h_unit) {
    LTWiFi_JoinStatus status = kLTWiFi_JoinStatus_Failed;
...
static int DriverWiFi_SendFrame(LTDeviceUnit h_unit, LTWiFi_Frame *frame) {
    LT_UNUSED(h_unit);
    LT_UNUSED(frame); // for non 6P3 builds

    return 0; // !!! should return valid code
}
...
static void DriverWiFi_OnDestroyHandle(LTHandle h_unit) {
    DriverWiFi_SetDriverState(h_unit, kLTWiFi_DriverState_Down);
}

define_LTLIBRARY_INTERFACE(LTDriverWiFi, DriverWiFi_OnDestroyHandle)
