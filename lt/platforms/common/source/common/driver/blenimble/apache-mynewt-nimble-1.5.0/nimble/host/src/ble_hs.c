...
static int ble_ctrl_reset_reason;
...
ble_hs_sched_reset(int reason)
{
    ...
    ble_ctrl_reset_reason = reason;
    ble_npl_eventq_put(ble_hs_evq, &ble_hs_ev_reset);
}

int
ble_hs_controller_get_reset_reason(void)
{
    int reason = ble_ctrl_reset_reason;
    ble_ctrl_reset_reason = 0;
    return reason;
}
...
extern void ble_npl_event_deinit(struct ble_npl_event *ev);
extern void ble_gatts_stop(void);
extern void ble_npl_callout_deinit(struct ble_npl_callout *c);
extern void ble_gap_deinit(void);
extern void ble_hs_hci_deinit(void);
extern void ble_hs_flow_stop(void);
