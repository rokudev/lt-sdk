...
#ifdef ROKU_NIMBLE_TCM_RAM
    __attribute__((section(".tcm_data")))static uint8_t pool_cmd_buf[ OS_MEMPOOL_BYTES(POOL_CMD_COUNT, POOL_CMD_SIZE) ];
    __attribute__((section(".tcm_data")))static uint8_t pool_evt_lo_buf[ OS_MEMPOOL_BYTES(POOL_EVT_LO_COUNT, POOL_EVT_SIZE) ];
    __attribute__((section(".tcm_data")))static uint8_t pool_evt_buf[ OS_MEMPOOL_BYTES(POOL_EVT_COUNT, POOL_EVT_SIZE) ];
    __attribute__((section(".tcm_data")))static uint8_t pool_acl_buf[ OS_MEMPOOL_BYTES(POOL_ACL_COUNT, POOL_ACL_SIZE) ];
#else
...
int ble_transport_deinit(void)
{
    int ret = 0;

    ret += os_mempool_clear(&pool_evt_lo);

    ret += os_mempool_clear(&pool_evt);

    ret += os_mempool_clear(&pool_cmd);

    pool_acl.mpe_put_cb = NULL;
    pool_acl.mpe_put_arg = NULL;
    ret += os_mempool_clear(&pool_acl.mpe_mp);
