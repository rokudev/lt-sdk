...
#ifdef ROKU_LTOS
/**
 * Sends the controller an HCI buffer.
*/

int ble_hs_hci_send_cmd(uint16_t opcode, const uint8_t *buffer, uint16_t len, uint8_t *rsp, uint16_t rsplen);
int ble_hs_hci_send_cmd_no_resp(uint16_t opcode, const uint8_t *cmd, uint16_t len);
#endif
#ifdef __cplusplus
}
#endif
