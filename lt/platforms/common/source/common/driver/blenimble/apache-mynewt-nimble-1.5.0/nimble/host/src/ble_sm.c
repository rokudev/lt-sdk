...
ble_sm_ltk_req_rx(const struct ble_hci_ev_le_subev_lt_key_req *ev)
{
    ...
    if (restore) {
        ...
        // Race condition. Slave connection arrives just after Application issues a Stop Advertisement
        if (!conn) {
            LTLOG_YELLOWALERT("ltk.req.rx", "BR-642:NULL conn. Reset controller");
            ble_hs_sched_reset(BLE_HS_ECONTROLLER);
            ble_hs_unlock();

            res.app_status = BLE_HS_ECONTROLLER;
            ble_sm_process_result(conn_handle, &res);
