...
#ifdef ROKU_LTOS
int
ble_hs_hci_send_cmd_no_resp(uint16_t opcode, const uint8_t *cmd, uint16_t len)
{
    return ble_hs_hci_cmd_tx_no_rsp(opcode, cmd, len);
}

int
ble_hs_hci_send_cmd(uint16_t opcode, const uint8_t *cmd, uint16_t len, uint8_t *rsp, uint16_t rsplen)
{
    return ble_hs_hci_cmd_tx(opcode, cmd, len, rsp, rsplen);
}
#endif